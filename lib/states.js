...
const STATE_META = {
    ...
    'status.data_age_seconds': { unit: 's', role: 'value.interval', type: 'number' },
    ...
    'charging.evcc_status': { role: 'text', type: 'string' },
    ...
    'charging.battery_power_kw': { unit: 'kW', role: 'value.power', type: 'number' },
    'charging.ac_gun_connected': { role: 'indicator.connected', type: 'boolean' },
    ...
    'diagnostics.tire_pressure_all_ok': { role: 'indicator', type: 'boolean' },
    'diagnostics.data_is_stale': { role: 'indicator', type: 'boolean' },

    'notifications.unread_count': { role: 'value', type: 'number' },
    'notifications.last_message_title': { role: 'text', type: 'string' },

    'abrp.enabled': { role: 'indicator', type: 'boolean' },
    'abrp.status': { role: 'text', type: 'string' },
    'abrp.success': { role: 'indicator', type: 'boolean' },
    'abrp.last_push': { role: 'value.time', type: 'number' },
    'abrp.error': { role: 'text', type: 'string' },
    'abrp.telemetry_keys': { role: 'text', type: 'string' },
};
...
const CONTROLS = [
    ...
    // ---- Zusätzliche Befehle aus markoceri/leapmotor-api (noch nicht am Fahrzeug verifiziert) ----
    {
        id: 'charging',
        name: 'Laden starten (true) / stoppen (false)',
        type: 'boolean',
        role: 'switch',
        handler: (client, vin, value) => (value ? client.chargeStart(vin) : client.chargeStop(vin)),
    },
    {
        id: 'sentry_mode',
        name: 'Wächtermodus ein/aus',
        type: 'boolean',
        role: 'switch',
        handler: (client, vin, value) => (value ? client.sentryModeOn(vin) : client.sentryModeOff(vin)),
    },
    {
        id: 'healthy_charging',
        name: 'Schonendes Laden ein/aus',
        type: 'boolean',
        role: 'switch',
        handler: (client, vin, value) => (value ? client.healthyChargingOn(vin) : client.healthyChargingOff(vin)),
    },
    {
        id: 'on3',
        name: 'ON3-Modus ein/aus',
        type: 'boolean',
        role: 'switch',
        handler: (client, vin, value) => (value ? client.on3On(vin) : client.on3Off(vin)),
    },
    {
        id: 'fuel_heating',
        name: 'Standheizung ein/aus (EREV)',
        type: 'boolean',
        role: 'switch',
        handler: (client, vin, value) => (value ? client.fuelHeatingOn(vin) : client.fuelHeatingOff(vin)),
    },
    {
        id: 'sunroof_open',
        ...
        type: 'boolean',
        role: 'button',
        handler: (client, vin) => client.openSunroof(vin),
    },
    {
        id: 'sunroof_close',
        ...
        type: 'boolean',
        role: 'button',
        handler: (client, vin) => client.closeSunroof(vin),
    },
    {
        id: 'hotspot',
        name: 'WLAN-Hotspot aktivieren',
        type: 'boolean',
        role: 'button',
        handler: (client, vin) => client.hotspot(vin),
    },
    {
        id: 'autopark',
        name: 'Einparkassistent / Summon',
        type: 'boolean',
        role: 'button',
        handler: (client, vin) => client.autopark(vin),
    },
    {
        id: 'ble_key_restart',
        name: 'Bluetooth-Schlüssel neu starten',
        type: 'boolean',
        role: 'button',
        handler: (client, vin) => client.bleKeyRestart(vin),
    },
    {
        id: 'speed_limit',
        name: 'Tempolimit (km/h)',
        type: 'number',
        role: 'level.speed',
        unit: 'km/h',
        min: 0,
        max: 200,
        handler: (client, vin, value) => client.setSpeedLimit(vin, parseInt(value, 10)),
    },
    {
        id: 'music',
        name: 'Musik (play/pause/next/previous)',
        type: 'string',
        role: 'text',
        handler: (client, vin, value) => client.musicControl(vin, String(value)),
    },
    {
        id: 'video',
        name: 'Video (play/pause/next/previous)',
        type: 'string',
        role: 'text',
        handler: (client, vin, value) => client.videoControl(vin, String(value)),
    },

    // ---- Bereits portierte, parametrische Befehle als JSON-State ----
    {
        id: 'send_destination',
        name: 'Navigationsziel senden (JSON: {address, addressName, latitude, longitude})',
        type: 'string',
        role: 'json',
        handler: (client, vin, value) => {
            const d = typeof value === 'string' ? JSON.parse(value) : value;
            return client.sendDestination(vin, {
                address: d.address,
                addressName: d.addressName || d.address,
                latitude: d.latitude,
    ...
    {
        id: 'prepare_car',
        name: 'Fahrzeug vorbereiten (JSON-Optionen oder leer für Standard-Klima)',
        type: 'string',
        role: 'json',
        handler: (client, vin, value) => {
            const opts = typeof value === 'string' && value.trim() ? JSON.parse(value) : {};
            return client.prepareCar(vin, opts);
        },
    },
    {
        id: 'climate_schedule',
        name: 'Klima-Zeitplan (JSON: {startTime, mode, temperature, ...})',
        type: 'string',
        role: 'json',
        handler: (client, vin, value) => {
            const opts = typeof value === 'string' ? JSON.parse(value) : value;
            return client.setClimateSchedule(vin, opts);
        },
    },
