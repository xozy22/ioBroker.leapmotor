...
module.exports = function attachCommands(Client, helpers) {
    ...
    // ---- Zusätzliche Aktionen aus markoceri/leapmotor-api (noch nicht am Fahrzeug verifiziert) ----

    P.chargeStart = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_CHARGE_START });
    };
    P.chargeStop = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_CHARGE_STOP });
    };
    P.sentryModeOn = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_SENTRY_MODE_ON });
    };
    P.sentryModeOff = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_SENTRY_MODE_OFF });
    };
    P.healthyChargingOn = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_HEALTHY_CHARGING_ON });
    };
    P.healthyChargingOff = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_HEALTHY_CHARGING_OFF });
    };
    P.on3On = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_ON3_ON });
    };
    P.on3Off = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_ON3_OFF });
    };
    P.fuelHeatingOn = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_FUEL_HEATING_ON });
    };
    P.fuelHeatingOff = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_FUEL_HEATING_OFF });
    };
    P.openSunroof = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_SUNROOF_OPEN });
    };
    P.closeSunroof = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_SUNROOF_CLOSE });
    };
    P.hotspot = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_HOTSPOT });
    };
    P.autopark = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_AUTOPARK });
    };
    P.bleKeyRestart = function (vin) {
        return this._remoteControl({ vin, action: C.REMOTE_CTL_BLE_KEY_RESTART });
    };
    P.setSpeedLimit = function (vin, kmh) {
        // value-Mechanismus erzeugt {"value":"<kmh>"}
        return this._remoteControl({ vin, action: C.REMOTE_CTL_SPEED_LIMIT, value: parseInt(kmh, 10) });
    };
    P.musicControl = function (vin, operation) {
        return this._remoteControl({
            vin,
            action: C.REMOTE_CTL_MUSIC,
            cmdContent: { operation: String(operation) },
        });
    };
    P.videoControl = function (vin, operation) {
        return this._remoteControl({
            vin,
            action: C.REMOTE_CTL_VIDEO,
            cmdContent: { operation: String(operation) },
        });
    };
