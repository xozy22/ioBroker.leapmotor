...
// Schwelle, ab der die Cloud-Daten als veraltet gelten (wie im HA-Original: 15 Min).
const STALE_THRESHOLD_SECONDS = 900;

/**
 * Alter der zwischengespeicherten Cloud-Daten in Sekunden
 * (jetzt minus dem Zeitstempel der letzten Fahrzeugmeldung).
 *
 * @param {*} stsValue - signal.sts (collectTime des Fahrzeugs)
 * @param {number} nowMs - aktueller Zeitstempel in ms
 * @returns {number|null}
 */
function dataAgeSeconds(stsValue, nowMs) {
    const sts = safeFloat(stsValue);
    if (sts == null || sts <= 0) {
        return null;
    }
    // collectTime kann in Sekunden (10-stellig) oder Millisekunden (13-stellig) vorliegen
    const tsMs = sts > 1e12 ? sts : sts * 1000;
 ...
 * @param {number} [nowMs] - aktueller Zeitstempel in ms (injizierbar für Tests)
 * @returns {object}
 */
function normalizeVehicle(bundle, userId, nowMs = Date.now()) {
    ...
    const dataAge = dataAgeSeconds(signal.sts, nowMs);
    ...
    return {
        ...
        diagnostics: {
            ...
            data_is_stale: dataAge != null ? dataAge > STALE_THRESHOLD_SECONDS : null,
